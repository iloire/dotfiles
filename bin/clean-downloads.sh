...
if [ "$DRY_RUN" = false ]; then
    WATCHDOG_METADATA="{\"machine\":\"$(hostname)\",\"files_deleted\":$TOTAL_FILES_DELETED,\"dirs_deleted\":$TOTAL_DIRS_DELETED,\"bytes_freed\":$TOTAL_BYTES_FREED}" \
        $HOME/dotfiles/bin/send-watchdog "maintenance" "downloads.cleaned" "info" "Deleted $TOTAL_FILES_DELETED files, freed $(format_bytes $TOTAL_BYTES_FREED)"
fi
